  ...
  sessionNamespace.on('connection', (socket) => {
    ...
    socket.on('session:leave', (data) => {
      try {
        ...
        // Notify user
        socket.emit('session:left', { sessionId, message: 'Successfully left session' });
    ...
    socket.on('session:offer', (data) => {
      try {
        ...
        console.log(`🎥 Offer sent from ${socket.user.id} to ${targetSocketId}`);
      } catch (error) {
    ...
    socket.on('session:answer', (data) => {
      try {
        ...
        console.log(`🎥 Answer sent from ${socket.user.id} to ${targetSocketId}`);
      } catch (error) {
     ...
     */
    socket.on('session:toggle-video', (data) => {
      try {
        const { sessionId, enabled } = data;

        if (!sessionId || typeof enabled !== 'boolean') {
          socket.emit('session:error', { message: 'Invalid toggle video data' });
          return;
        }

        // Broadcast video state change to other participants
        socket.to(`session:${sessionId}`).emit('session:participant-video-toggled', {
          userId: socket.user.id,
          userName: socket.userName,
          videoEnabled: enabled,
        });

        console.log(`🎥 User ${socket.user.id} video: ${enabled ? 'ON' : 'OFF'}`);
      } catch (error) {
     ...
     * Payload: { sessionId: string, enabled: boolean }
     */
    socket.on('session:toggle-audio', (data) => {
      try {
        const { sessionId, enabled } = data;

        if (!sessionId || typeof enabled !== 'boolean') {
          socket.emit('session:error', { message: 'Invalid toggle audio data' });
          return;
        }

        // Broadcast audio state change to other participants
        socket.to(`session:${sessionId}`).emit('session:participant-audio-toggled', {
          userId: socket.user.id,
          userName: socket.userName,
          audioEnabled: enabled,
        });

        console.log(`🎥 User ${socket.user.id} audio: ${enabled ? 'ON' : 'OFF'}`);
      } catch (error) {
     ...
     * Payload: { sessionId: string }
     */
    socket.on('session:screen-share-start', (data) => {
      try {
        const { sessionId } = data;
        ...
        socket.to(`session:${sessionId}`).emit('session:participant-screen-sharing', {
          userId: socket.user.id,
          userName: socket.userName,
          isSharing: true,
        });
     ...
     * Payload: { sessionId: string }
     */
    socket.on('session:screen-share-stop', (data) => {
      try {
        const { sessionId } = data;
    ...
    socket.on('session:chat-message', (data) => {
      try {
        ...
        sessionNamespace.to(`session:${sessionId}`).emit('session:chat-message', {
          userId: socket.user.id,
          userName: socket.userName,
          message,
        ...
        console.log(`💬 Chat message from ${socket.user.id} in session ${sessionId}`);
      } catch (error) {
    ...
    socket.on('session:raise-hand', (data) => {
      try {
        ...
        sessionNamespace.to(`session:${sessionId}`).emit('session:participant-hand-raised', {
          userId: socket.user.id,
          userName: socket.userName,
          handRaised: raised,
          timestamp: new Date().toISOString(),
        ...
        console.log(`✋ User ${socket.user.id} hand: ${raised ? 'RAISED' : 'LOWERED'}`);
      } catch (error) {
    ...
    socket.on('disconnect', () => {
      console.log(`🎥 User ${socket.user.id} disconnected from session namespace`);

      // Notify participants in the session
      if (socket.sessionId) {
        socket.to(`session:${socket.sessionId}`).emit('session:participant-left', {
          userId: socket.user.id,
          userName: socket.userName,
        });
      }
