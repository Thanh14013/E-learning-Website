...
const CommentItem = ({ comment, discussionId, depth = 0 }) => {
  const { user } = useAuth();
  const { toggleLikeComment, deleteComment, createComment, fetchDiscussionDetail } = useDiscussions();
  
  const [showReplyForm, setShowReplyForm] = useState(false);
  const [showDeleteConfirm, setShowDeleteConfirm] = useState(false);
  const [isLiked, setIsLiked] = useState(comment.likes?.includes(user?.id) || false);
